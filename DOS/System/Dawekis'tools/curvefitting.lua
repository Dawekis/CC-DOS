...
local str_3 = {}
local str_4 = {}
for i in io.lines("DOS/System/Dawekis'tools/UserData/Curve_fitting_config.txt") do
    str_1[j] = i
    j = j+1
end
j = 1
...
cat.Basalt.Button_Text(sidebar,14,11,"OK",colors.red,colors.black,function ()
    local j = 1
    local updata = io.open("DOS/System/Dawekis'tools/UserData/Curve_fitting_config.txt","w")
    io.output(updata)
    local str = {}
...
mainFrame:onEvent(function(event, side, channel, replyChannel, message, distance)
    local min = 999999
    local max = 0
    ...
    for i in io.lines("DOS/System/Dawekis'tools/UserData/Data.txt") do
